...
const ExtractedContentSchema = z.union([YouTubeContentSchema, NaverBlogContentSchema]);
  ...
  describe('POST /extract/auto', () => {
    ...
    it('should handle platform detection edge cases', async () => {
      ...
      for (const url of edgeCaseUrls) {
        ...
        expect(responseBody.success).toBe(false);
        expect(responseBody.error).toBeDefined();
        expect(['INVALID_URL', 'UNSUPPORTED_PLATFORM', 'CONTENT_NOT_FOUND'])
    ...
    it('should handle concurrent auto-detection requests', async () => {
      ...
      for (let i = 0; i < responses.length; i++) {
        ...
        // 플랫폼별 올바른 감지 확인
        if (concurrentRequests[i].url.includes('youtube') || concurrentRequests[i].url.includes('youtu.be')) {
          expect(responseBody.data.platform).toBe('youtube');
